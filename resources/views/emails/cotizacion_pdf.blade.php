<!DOCTYPE html>
<html lang="es">
<head><meta charset="utf-8"><title>Cotización {{ $folio }}</title>
<style>
...
<div class="header">
  ...
  <p><strong>Cotización {{ $folio }}</strong> | Fecha: {{ $fecha }}</p>
</div>
...
<div class="section">
  ...
  <table>
    <tr><td width="30%"><strong>Tipo:</strong></td><td>{{ $tipo_servicio }}</td></tr>
    <tr><td><strong>Origen:</strong></td><td>{{ $origen }}</td></tr>
    <tr><td><strong>Destino:</strong></td><td>{{ $destino }}</td></tr>
    <tr><td><strong>Distancia:</strong></td><td>{{ $distancia }} km</td></tr>
  </table>
</div>
...
<div class="section">
  ...
  <table>
    ...
    <tr><td>KM Excedentes ({{ $km_excedente }} km × ${{ $costo_km }}/km)</td><td align="right">${{ $costo_km_extra_total }}</td></tr>
    <tr><td>Casetas</td><td align="right">${{ $casetas }} {{ $incluye_peajes ? '(incluidas)' : '' }}</td></tr>
    ...
    <tr class="total-row"><td><strong>TOTAL ESTIMADO</strong></td><td align="right"><strong>${{ $total }}</strong></td></tr>
  </table>
</div>

<div class="footer">
  <p>Cotización sujeta a aprobación. Para confirmar, ingresa a tu portal con el enlace incluido en el correo.</p>
